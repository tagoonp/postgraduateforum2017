...
if(isset($_SESSION[$sprefix.'Username'])){
  ...
  $resultDel = $db->delete($strSQL, array(session_id(), $_SESSION[$sprefix.'Username']));
  ...
  if (!empty($_FILES)) {
      ...
      $strSQL = "INSERT INTO t5iw_fileupload (	fname,	sess_id, user_upload,	date_upload)
              VALUES (?,?,?,?) ";
      $result_update = $db->insert($strSQL, array($filename , session_id(), $_SESSION[$sprefix.'Username'], date('Y-m-d')));
  }
}
